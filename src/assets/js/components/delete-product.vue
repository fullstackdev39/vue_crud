...
<script>
    ...
    export default{
        ...
        methods: {
            getProduct: function()
            {
                this.$http.get('http://localhost:8080/api/product/' + this.$route.params.id).then((response) => {
            ...
            {
                this.$http.delete('http://localhost:8080/api/product/delete/' + this.$route.params.id, this.product, {
                    headers : {
